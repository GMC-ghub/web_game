...
class Room(object):
	...
	def go(self, direction):
		return self.paths.get(direction, None) #dict.get(key，default) 返回字典中指定键的值，如果指定的
		                                       #的键不存在，则返回默认值default
...
now need to pick one to take. Some of them could be damaged but you don't have time to look. There's 5 pods, which one do you take?
""")
...
back and see your ship implode then explode like a bright star, taking out the Gothon ship at the same time. 
You won!
...
"""
You jump into a random pod and hit the eject button. The pod escapes out into the void of space, 
then implodes as the hull ruptures, crushing your body into jam jelly.

""")

bomb_code_count = Room("Wrong Code",
"""
Maybe, you need tips.
"""
)
